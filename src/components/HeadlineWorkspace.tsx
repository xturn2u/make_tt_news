...
function versionText(data:NewsVersion){const t=data.triple_headline;return[t.line1,t.line2,t.line3].map(v=>String(v||'').trim()).filter(Boolean).join('\n')||data.header}
function wrapLines(ctx:CanvasRenderingContext2D,text:string,maxWidth:number,maxLines:number){const words=String(text||'').split(/\s+/).filter(Boolean),lines:string[]=[];let line='';for(const word of words){const test=line?`${line} ${word}`:word;if(ctx.measureText(test).width>maxWidth&&line){lines.push(line);line=word;if(lines.length===maxLines)break}else line=test}if(lines.length<maxLines&&line)lines.push(line);const used=lines.join(' ').split(/\s+/).filter(Boolean).length;if(used<words.length&&lines.length){let last=lines[lines.length-1];while(last&&ctx.measureText(last+' …').width>maxWidth)last=last.split(' ').slice(0,-1).join(' ');lines[lines.length-1]=(last||'')+' …'}return lines}
...
export function HeadlineWorkspace({project,setProject,version}:Props){
  ...
  const capture=async()=>{
    ...
    if(document.fonts?.ready)await document.fonts.ready;
    const canvas=await html2canvas(stageRef.current,{scale:ORIGINAL_CAPTURE_SCALE,backgroundColor:null,useCORS:true,logging:false});
  ...
  return <div className="workspace-section original-headline-workspace compact-headline-workspace">
    ...
    <div className="newspaper-section"><div className="section-heading"><div><span>Artikel-Screenshots</span><h3>Zeitungsausschnitte aus {version.toUpperCase()}</h3><p>Überschrift, Beschreibung ohne Hashtags und die ersten 40 Wörter des Sprechtexts.</p></div><button className="primary" disabled={busy} onClick={()=>void saveAll()}><Newspaper size={15}/>Alle 3 speichern</button></div><div className="newspaper-grid">{cards.map(card=><article key={card.key} className="newspaper-card"><img src={card.url} alt={`${card.style.name} Artikelausschnitt`}/><div><strong>{card.style.name}</strong><small>{card.style.hint}</small><div className="article-actions"><button className="primary" onClick={()=>void saveArticle(card)}>Als Headline speichern</button><button onClick={()=>download(card.url,card.filename)}>PNG</button></div></div></article>)}</div></div>
  </div>;
}
