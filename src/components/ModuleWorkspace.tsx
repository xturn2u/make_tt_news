...
import { HeadlineWorkspace, seedGeneratedAssets } from './HeadlineWorkspace';
...
function JsonImport({project,setProject}:SharedProps){
  const [json,setJson]=useState('');const [state,setState]=useState('');const [busy,setBusy]=useState(false);const fileRef=useRef<HTMLInputElement>(null);
  const apply=async(value=json)=>{setBusy(true);try{const newsPackage=parsePackageJson(value);const versions=availableVersionKeys(newsPackage);const first=versions[0]||'v1';const generated=await seedGeneratedAssets(newsPackage);setProject(current=>({...current,newsPackage,activeVersion:first,versionFlows:createVersionFlows(newsPackage),assets:[...generated,...current.assets.filter(asset=>!String(asset.source||'').startsWith('headline-generator')&&!String(asset.source||'').startsWith('article-generator:'))],workflowRevision:current.workflowRevision+1}));setState(`✓ ${newsPackage.meta.topic} · ${versions.length} Version${versions.length===1?'':'en'} · ${generated.length} Start-Assets erzeugt`)}catch(error){setState(error instanceof Error?error.message:String(error))}finally{setBusy(false)}};
  return <div className="workspace-section"><div className="callout"><FileJson size={18}/><div><b>JSON ist der Projekteinstieg</b><span>Jede vorhandene Version erzeugt automatisch einen eigenen Produktionsstrang. Schlagzeile und Zeitungsausschnitte werden direkt als Assets vorbereitet.</span></div></div><textarea className="json-editor" value={json} onChange={e=>setJson(e.target.value)} placeholder={'{\n  "package": {\n    "source_url": "https://…",\n    "meta": { "topic": "…", "breaking": false },\n    "versions": { "v1": {}, "v2": {}, "v3": {} }\n  }\n}'}/><div className="action-row"><button className="primary" disabled={busy} onClick={()=>void apply()}>{busy?'Import läuft…':'JSON importieren'}</button><button disabled={busy} onClick={()=>fileRef.current?.click()}><Upload size={15}/>Datei wählen</button></div><input ref={fileRef} hidden type="file" accept="application/json,.json" onChange={async e=>{const f=e.target.files?.[0];if(!f)return;const value=await f.text();setJson(value);void apply(value)}}/>{state&&<div className={`message ${state.startsWith('✓')?'ok':'error'}`}>{state}</div>}{project.newsPackage&&<div className="summary-card"><span>Aktuelles Paket</span><strong>{project.newsPackage.meta.topic}</strong><small>{availableVersionKeys(project.newsPackage).map(v=>v.toUpperCase()).join(' · ')}</small></div>}</div>
}

function CopyButton({value,label,onCopied}:{value:string;label:string;onCopied:(label:string)=>void}){
  return <button type="button" className="copy-field-btn" title={`${label} kopieren`} onClick={async()=>{try{await navigator.clipboard.writeText(value);onCopied(label)}catch{const area=document.createElement('textarea');area.value=value;document.body.append(area);area.select();document.execCommand('copy');area.remove();onCopied(label)}}}><Copy size={13}/>Kopieren</button>
}

function NewspackageWorkspace({project,setProject,version}:VersionProps){
  const data=project.newsPackage?.versions[version];const [copied,setCopied]=useState('');if(!data)return <Empty text={`${version.toUpperCase()} ist im Paket nicht vorhanden.`}/>;
  ...
  const copiedNow=(label:string)=>{setCopied(label);window.setTimeout(()=>setCopied(''),1200)};
  return <div className="workspace-section newspackage-workspace"><div className="version-context"><b>{version.toUpperCase()}</b><span>{data.header}</span></div><div className="field-grid newspackage-fields">
    <label className="wide copy-field"><span className="field-title">Header</span><CopyButton value={data.header} label="Header" onCopied={copiedNow}/><input value={data.header} onChange={e=>patch({header:e.target.value})}/></label>
    <label className="wide copy-field"><span className="field-title">Sprechtext <small>{words} Wörter</small></span><CopyButton value={data.speech_text} label="Sprechtext" onCopied={copiedNow}/><textarea className="tall" value={data.speech_text} onChange={e=>patch({speech_text:e.target.value})}/></label>
    <label className="wide copy-field"><span className="field-title">Beschreibung</span><CopyButton value={data.description} label="Beschreibung" onCopied={copiedNow}/><textarea value={data.description} onChange={e=>patch({description:e.target.value})}/></label>
    <label className="wide copy-field"><span className="field-title">Bildprompt</span><CopyButton value={data.image_prompt} label="Bildprompt" onCopied={copiedNow}/><textarea value={data.image_prompt} onChange={e=>patch({image_prompt:e.target.value})}/></label>
    ...
    <label className="wide copy-field"><span className="field-title">Hashtags</span><CopyButton value={data.hashtags.join(' ')} label="Hashtags" onCopied={copiedNow}/><input value={data.hashtags.join(' ')} onChange={e=>patch({hashtags:e.target.value.split(/\s+/).filter(Boolean).slice(0,5)})}/></label>
