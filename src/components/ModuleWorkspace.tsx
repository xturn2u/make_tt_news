...
function NewspackageWorkspace({project,setProject,version}:VersionProps){
  ...
  return <div className="workspace-section newspackage-workspace"><div className="version-context"><b>{version.toUpperCase()}</b><span>{data.header}</span></div><div className="field-grid newspackage-fields">
    <CopyField label="Header" value={data.header} onCopied={copiedText}><input value={data.header} onChange={e=>patch({header:e.target.value})}/></CopyField>
    <CopyField label={`Sprechtext · ${words} Wörter`} value={data.speech_text} onCopied={copiedText}><textarea className="tall" value={data.speech_text} onChange={e=>patch({speech_text:e.target.value})}/></CopyField>
    <CopyField label="Beschreibung" value={data.description} onCopied={copiedText}><textarea value={data.description} onChange={e=>patch({description:e.target.value})}/></CopyField>
