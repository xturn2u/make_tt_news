...
export function ResearchWorkspace({project,setProject}:Props){
  ...
  const patchQuery=(version:VersionKey,value:string)=>{if(!setProject)return;setProject(current=>{const previous=current.research[version];return {...current,research:{...current.research,[version]:{query:value,imageSearchUrl:`https://www.google.com/search?tbm=isch&q=${encodeURIComponent(value)}`,links:previous?.links||[],status:'idle'}}}})};
